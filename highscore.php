...
<p><a href="?seite=highscore&highscore=staedte">St&#228;dte-Highscore</a></p>
...
elseif($highscore == 'spieler')
{
	echo '<tr><th colspan="5">Spieler Highscore</th></tr>';
	echo '<tr><td>#</td><td>Spieler</td><td>St&#228;dtepunkte</td>';
	echo '<td><a href="?seite=highscore&highscore=forscher">Forschungspunkte</a></td><td>Gesamtpunkte</td></tr>';

	$gamer_query = mysql_query("SELECT id,user,buildings_points,research_points,points FROM gamer ORDER BY points DESC,research_points DESC LIMIT 100");
	$x = 0;
...
elseif($highscore == 'forscher')
{
	echo '<tr><th colspan="5">Spieler Highscore</th></tr>';
	echo '<tr><td>#</td><td>Spieler</td><td>St&#228;dtepunkte</td><td>Forschungspunkte</td>';
	echo '<td><a href="?seite=highscore&highscore=spieler">Gesamtpunkte</a></td></tr>';

	$gamer_query = mysql_query("SELECT id,user,buildings_points,research_points,points FROM gamer ORDER BY research_points DESC,points DESC LIMIT 100");
	$x = 0;
	while($gamer_row = mysql_fetch_object($gamer_query))
	{
		$x++;
